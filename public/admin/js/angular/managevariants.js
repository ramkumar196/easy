function ManageVariantsController ($scope, $http, $log, $q,category_services,commonServices,alertify,$window) {
        $scope.redirect = function(data)
        {
            window.location.href = '/variants/'+data+'/edit';			
        
        };

        $scope.categoryList=function()
        {
            commonServices.selectAllList('variants').then(function(res){
        ...
        $scope.statusList = $window.statusList;
            ...
            $scope.updateStatus=function(id,data)
            {
                ...
                alertify.confirm("Are you sure ?", function () {
                    commonServices.updateStatus('variants',id,dataArray).then(function(res)
                    {
                        var d = res.data;
                        if(d.http_code == 404) {
                        return;
    ...
    app.controller('ManageVariantsController',ManageVariantsController);
